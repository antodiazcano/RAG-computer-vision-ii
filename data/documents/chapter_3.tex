...
\begin{figure}[H]
\centering
\includegraphics[scale=0.62]{images/semseg}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.9]{images/naivesem}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{images/tc}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{images/upmethods}
\caption{Example of bed of nails and nearest neighbor.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[scale=1.75]{images/maxunp}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.71]{images/fcn}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.42]{images/unet}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.7]{images/st}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.8]{images/st_created}
...
In order to extract the content features and style features of the image, we can select the output of certain layers in the network. Generally speaking, \textbf{the closer to the input layer, the easier to extract details of the image, and the closer to the output layer, the easier to extract the global information of the image} (an example is shown in Figure \ref{features}). In order to avoid excessively retaining the details of the content image in the synthesized image,  usually a layer that is closer to the output is chosen as the \textbf{content layer} to output the content features of the image. It is also common to select the output of different layers for extracting local and global style features. These layers are also called \textbf{style layers}.

\begin{figure}[H]
\centering
\includegraphics[scale=0.28]{images/features}
\caption{Example of a CNN trained to classify emotions in faces. The closer to the input layer, the easier to extract details of the image, and the closer to the output layer, the easier to extract the global information of the image.}
\label{features}
\end{figure}

Since there is no need to update the model parameters of the pretrained network during training, \textbf{we can extract the content and the style features even before the training starts}, and therefore during training we only need to perform forward passes in the synthesized image, not in the content or style images.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{images/wloss}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.25]{images/image_segm}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{images/comparation_segmentation}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.23]{images/keypoint}
...
\begin{question}\label{ex_proof_1}
...
\begin{question}\label{ex_proof_2}
Show that if we have an arbitrary $2\times 2$ matrix $M$, let's say
$$M=\begin{pmatrix}
    a & b \\
    c & d
